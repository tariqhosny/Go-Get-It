    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if let newValue = change?[.newKey] {
            ...
            if self.currentURL == "https://e-bakers.org/About-Us"{
                ...
                orderApi.createOrderApi(totalPrice: self.totalPrice, phone: phone, region: region, city: city, country: country, street: street, latidude: lat, langitude: lang, department: apartmenetNumber, floor_number: floorNumber, home_number: buildNumber, method: 2) { (error: Error?, success: Bool?) in
                    if success == true{
